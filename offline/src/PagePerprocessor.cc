...
inline static void calculateWeight(const unordered_map<string, vector<int>>& forDF, 
    ...
    for (size_t idx = 0; idx < allTF.size(); ++idx) {
        ...
        for (const auto& pair : page) {
            ...
            double weight = TF_IDF(TF, forDF.at(word).size(), numOfArticles);
...
void PagePreprocessor::buildInvertIndexMap() {
    ...
    // 词 词在全部文章中出现次数 词在哪些文章id中出现过 // 原来DF是词出现的文章的数量，而不是词在全部的文章中出现的次数，我c！
    //unordered_map<string, pair<int, vector<int>>> forDF; 

    // 词 词在哪些文章(id - 1)中出现过  vetcor<int>.size() = DF 
    unordered_map<string, vector<int>> forDF;
    
    // 每个下标都是一个文章，string:词 int:该词在当前文章中出现的次数 , 下标0保存第一个文章
    ...
    while (std::getline(OffsetIfs, line)) {
        ...
        // 得到了当前文章的所有词在当前文章出现的次数
        allTF[idx] = forTF;
        
        for (const pair<string, int>& TFpair : forTF) {
            const string& word = TFpair.first;
            forDF[word].push_back(idx);
        }
